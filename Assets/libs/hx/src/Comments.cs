// Generated by Haxe 4.0.2+606b176a3
using haxe.root;

#pragma warning disable 109, 114, 219, 429, 168, 162
namespace haxe.root {
	public class Comments : global::haxe.lang.HxObject {
		
		public Comments(global::haxe.lang.EmptyObject empty) {
		}
		
		
		public Comments() {
			#line 43 "C:\\projects\\BCGAME\\sangong\\src\\Comments.hx"
			global::haxe.root.Comments.__hx_ctor__Comments(this);
		}
		#line default
		
		protected static void __hx_ctor__Comments(global::haxe.root.Comments __hx_this) {
		}
		
		
		public static string extract(object e) {
			unchecked {
				#line 128 "C:\\projects\\BCGAME\\sangong\\src\\Comments.hx"
				global::haxe.macro.ExprDef _g = ((global::haxe.macro.ExprDef) (global::haxe.lang.Runtime.getField(e, "expr", 1126038837, true)) );
				#line 128 "C:\\projects\\BCGAME\\sangong\\src\\Comments.hx"
				if (( _g._hx_index == 0 )) {
					#line 129 "C:\\projects\\BCGAME\\sangong\\src\\Comments.hx"
					global::haxe.macro.Constant _g1 = ( _g as global::haxe.macro.ExprDef_EConst ).c;
					#line 129 "C:\\projects\\BCGAME\\sangong\\src\\Comments.hx"
					if (( _g1._hx_index == 2 )) {
						#line 129 "C:\\projects\\BCGAME\\sangong\\src\\Comments.hx"
						global::haxe.macro.StringLiteralKind _g3 = ( _g1 as global::haxe.macro.Constant_CString ).kind;
						#line 129 "C:\\projects\\BCGAME\\sangong\\src\\Comments.hx"
						string v = ( _g1 as global::haxe.macro.Constant_CString ).s;
						string temp = v;
						#line 131 "C:\\projects\\BCGAME\\sangong\\src\\Comments.hx"
						if (( temp.Length < 80 )) {
							#line 133 "C:\\projects\\BCGAME\\sangong\\src\\Comments.hx"
							string str80 = "                                                                                ";
							temp = global::haxe.lang.Runtime.concat(temp, global::haxe.lang.StringExt.substr(str80, 0, ( str80.Length - temp.Length )));
						}
						
						#line 136 "C:\\projects\\BCGAME\\sangong\\src\\Comments.hx"
						return temp;
					}
					else {
						#line 137 "C:\\projects\\BCGAME\\sangong\\src\\Comments.hx"
						return "not a proper comment";
					}
					
				}
				else {
					#line 137 "C:\\projects\\BCGAME\\sangong\\src\\Comments.hx"
					return "not a proper comment";
				}
				
			}
			#line default
		}
		
		
	}
}


