// Generated by Haxe 4.0.2+606b176a3
using haxe.root;

#pragma warning disable 109, 114, 219, 429, 168, 162
namespace hx.ws {
	public class Buffer : global::haxe.lang.HxObject {
		
		public Buffer(global::haxe.lang.EmptyObject empty) {
		}
		
		
		public Buffer() {
			#line 12 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			global::hx.ws.Buffer.__hx_ctor_hx_ws_Buffer(this);
		}
		#line default
		
		protected static void __hx_ctor_hx_ws_Buffer(global::hx.ws.Buffer __hx_this) {
			#line 10 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			__hx_this.chunks = new global::haxe.root.Array(new object[]{});
			#line 9 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			__hx_this.currentData = null;
			#line 8 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			__hx_this.currentOffset = 0;
			#line 7 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			__hx_this.length = 0;
			#line 6 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			__hx_this.available = 0;
			#line 12 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			{
			}
			
		}
		#line default
		
		public int available;
		
		public int length;
		
		public int currentOffset;
		
		public global::haxe.io.Bytes currentData;
		
		public global::haxe.root.Array chunks;
		
		public virtual void writeByte(int v) {
			unchecked {
				#line 16 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				global::haxe.io.Bytes b = global::haxe.io.Bytes.alloc(1);
				b.b[0] = ((byte) (v) );
				#line 18 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				this.writeBytes(b);
			}
			#line default
		}
		
		
		public virtual void writeShort(int v) {
			unchecked {
				#line 22 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				global::haxe.io.Bytes b = global::haxe.io.Bytes.alloc(2);
				b.b[0] = ((byte) (( ( v >> 8 ) & 255 )) );
				#line 24 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				b.b[1] = ((byte) (( v & 255 )) );
				this.writeBytes(b);
			}
			#line default
		}
		
		
		public virtual void writeInt(int v) {
			unchecked {
				#line 29 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				global::haxe.io.Bytes b = global::haxe.io.Bytes.alloc(4);
				b.b[0] = ((byte) (( ( v >> 24 ) & 255 )) );
				#line 31 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				b.b[1] = ((byte) (( ( v >> 16 ) & 255 )) );
				b.b[2] = ((byte) (( ( v >> 8 ) & 255 )) );
				#line 33 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				b.b[3] = ((byte) (( v & 255 )) );
				this.writeBytes(b);
			}
			#line default
		}
		
		
		public virtual void writeBytes(global::haxe.io.Bytes data) {
			#line 38 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			this.chunks.push(data);
			this.available += data.length;
			#line 40 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			this.length = this.available;
		}
		#line default
		
		public virtual global::haxe.io.Bytes readAllAvailableBytes() {
			#line 44 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			return this.readBytes(this.available);
		}
		#line default
		
		public virtual string readLine() {
			#line 48 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			global::haxe.io.Bytes bytes = this.readUntil("\n");
			if (( bytes == null )) {
				#line 50 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				return null;
			}
			
			#line 52 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			return bytes.toString().Trim();
		}
		#line default
		
		public virtual global::haxe.root.Array readLinesUntil(string delimiter) {
			#line 56 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			global::haxe.io.Bytes bytes = this.readUntil(delimiter);
			if (( bytes == null )) {
				#line 58 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				return null;
			}
			
			#line 60 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			return global::haxe.lang.StringExt.split(bytes.toString().Trim(), "\n");
		}
		#line default
		
		public virtual global::haxe.io.Bytes readUntil(string delimiter) {
			unchecked {
				#line 65 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				int dl = delimiter.Length;
				#line 67 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				{
					#line 67 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
					int _g = 0;
					#line 67 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
					int _g1 = ( this.available - dl );
					#line 67 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
					while (( _g < _g1 )) {
						#line 67 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						int i = _g++;
						bool matched = true;
						#line 69 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						{
							#line 69 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
							int _g2 = 0;
							#line 69 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
							int _g11 = dl;
							#line 69 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
							while (( _g2 < _g11 )) {
								#line 69 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
								int j = _g2++;
								if (global::haxe.lang.Runtime.eq(this.peekByte(( ( ( this.currentOffset + i ) + j ) + 1 )), global::haxe.lang.StringExt.charCodeAt(delimiter, j))) {
									#line 71 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
									continue;
								}
								
								#line 73 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
								matched = false;
								break;
							}
							
						}
						
						#line 77 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						if (matched) {
							#line 78 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
							global::haxe.io.Bytes bytes = this.readBytes(( ( i + dl ) + 1 ));
							return bytes;
						}
						
					}
					
				}
				
				#line 83 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				return null;
			}
			#line default
		}
		
		
		public virtual global::haxe.io.Bytes readBytes(int count) {
			#line 87 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			int count2 = ((int) (global::System.Math.Min(((double) (count) ), ((double) (this.available) ))) );
			global::haxe.io.Bytes @out = global::haxe.io.Bytes.alloc(count2);
			#line 89 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			{
				#line 89 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				int _g = 0;
				#line 89 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				int _g1 = count2;
				#line 89 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				while (( _g < _g1 )) {
					#line 89 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
					int n = _g++;
					#line 89 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
					{
						#line 89 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						int v = this.readByte();
						#line 89 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						@out.b[n] = ((byte) (v) );
					}
					
				}
				
			}
			
			#line 90 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			return @out;
		}
		#line default
		
		public virtual uint readUnsignedShort() {
			unchecked {
				#line 94 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				int h = this.readByte();
				int l = this.readByte();
				#line 96 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				return ((uint) (( ( h << 8 ) | l )) );
			}
			#line default
		}
		
		
		public virtual uint readUnsignedInt() {
			unchecked {
				#line 100 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				int v3 = this.readByte();
				int v2 = this.readByte();
				#line 102 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				int v1 = this.readByte();
				int v0 = this.readByte();
				#line 104 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				return ((uint) (( ( ( ( v3 << 24 ) | ( v2 << 16 ) ) | ( v1 << 8 ) ) | v0 )) );
			}
			#line default
		}
		
		
		public virtual int readByte() {
			#line 108 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			if (( this.available <= 0 )) {
				#line 108 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				throw global::haxe.lang.HaxeException.wrap("No bytes available");
			}
			
			#line 109 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			while (( ( this.currentData == null ) || ( this.currentOffset >= this.currentData.length ) )) {
				#line 110 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				this.currentOffset = 0;
				this.currentData = ((global::haxe.io.Bytes) (this.chunks.shift()) );
			}
			
			#line 113 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			this.available--;
			this.length = this.available;
			#line 115 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			return ((int) (((byte) (this.currentData.b[this.currentOffset++]) )) );
		}
		#line default
		
		public virtual int peekByte(int offset) {
			#line 119 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			if (( this.available <= 0 )) {
				#line 119 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				throw global::haxe.lang.HaxeException.wrap("No bytes available");
			}
			
			#line 120 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			int tempOffset = offset;
			global::haxe.io.Bytes tempData = ((global::haxe.io.Bytes) (this.chunks.__get(0)) );
			#line 122 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			if (( tempData == null )) {
				#line 123 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				tempData = this.currentData;
			}
			
			#line 125 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			int chunkIndex = 0;
			while (( tempOffset >= tempData.length )) {
				#line 127 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				tempOffset -= tempData.length;
				 ++ chunkIndex;
				#line 129 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				tempData = ((global::haxe.io.Bytes) (this.chunks.__get(chunkIndex)) );
			}
			
			#line 131 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			return ((int) (((byte) (tempData.b[tempOffset]) )) );
		}
		#line default
		
		public virtual int peekUntil(int @byte) {
			unchecked {
				#line 135 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				int tempOffset = this.currentOffset;
				global::haxe.io.Bytes tempData = ((global::haxe.io.Bytes) (this.chunks.__get(0)) );
				#line 137 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				if (( tempData == null )) {
					#line 138 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
					tempData = this.currentData;
				}
				
				#line 140 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				int chunkIndex = 0;
				while (( tempOffset >= tempData.length )) {
					#line 142 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
					tempOffset -= tempData.length;
					 ++ chunkIndex;
					#line 144 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
					tempData = ((global::haxe.io.Bytes) (this.chunks.__get(chunkIndex)) );
				}
				
				#line 146 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				while (( tempOffset < tempData.length )) {
					#line 147 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
					if (( ((int) (((byte) (tempData.b[tempOffset]) )) ) == @byte )) {
						#line 148 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return ( tempOffset + 1 );
					}
					
					#line 150 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
					 ++ tempOffset;
				}
				
				#line 153 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				return -1;
			}
			#line default
		}
		
		
		public virtual bool endsWith(string e) {
			#line 157 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			int i = ( this.available - e.Length );
			int n = this.currentOffset;
			#line 159 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			while (( i < this.available )) {
				#line 160 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				if (( ! (global::haxe.lang.Runtime.eq(this.peekByte(i), global::haxe.lang.StringExt.charCodeAt(e, n))) )) {
					#line 161 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
					return false;
				}
				
				#line 163 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				 ++ i;
				 ++ n;
			}
			
			#line 167 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			return true;
		}
		#line default
		
		public override double __hx_setField_f(string field, int hash, double @value, bool handleProperties) {
			unchecked {
				#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				switch (hash) {
					case 1134303404:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						this.currentOffset = ((int) (@value) );
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return @value;
					}
					
					
					case 520590566:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						this.length = ((int) (@value) );
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return @value;
					}
					
					
					case 2005096905:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						this.available = ((int) (@value) );
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return @value;
					}
					
					
					default:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return base.__hx_setField_f(field, hash, @value, handleProperties);
					}
					
				}
				
			}
			#line default
		}
		
		
		public override object __hx_setField(string field, int hash, object @value, bool handleProperties) {
			unchecked {
				#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				switch (hash) {
					case 1152047462:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						this.chunks = ((global::haxe.root.Array) (@value) );
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return @value;
					}
					
					
					case 2062888707:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						this.currentData = ((global::haxe.io.Bytes) (@value) );
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return @value;
					}
					
					
					case 1134303404:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						this.currentOffset = ((int) (global::haxe.lang.Runtime.toInt(@value)) );
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return @value;
					}
					
					
					case 520590566:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						this.length = ((int) (global::haxe.lang.Runtime.toInt(@value)) );
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return @value;
					}
					
					
					case 2005096905:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						this.available = ((int) (global::haxe.lang.Runtime.toInt(@value)) );
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return @value;
					}
					
					
					default:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return base.__hx_setField(field, hash, @value, handleProperties);
					}
					
				}
				
			}
			#line default
		}
		
		
		public override object __hx_getField(string field, int hash, bool throwErrors, bool isCheck, bool handleProperties) {
			unchecked {
				#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				switch (hash) {
					case 1538685534:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return ((global::haxe.lang.Function) (new global::haxe.lang.Closure(this, "endsWith", 1538685534)) );
					}
					
					
					case 1154061571:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return ((global::haxe.lang.Function) (new global::haxe.lang.Closure(this, "peekUntil", 1154061571)) );
					}
					
					
					case 401708611:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return ((global::haxe.lang.Function) (new global::haxe.lang.Closure(this, "peekByte", 401708611)) );
					}
					
					
					case 1763375486:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return ((global::haxe.lang.Function) (new global::haxe.lang.Closure(this, "readByte", 1763375486)) );
					}
					
					
					case 89107268:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return ((global::haxe.lang.Function) (new global::haxe.lang.Closure(this, "readUnsignedInt", 89107268)) );
					}
					
					
					case 1997218321:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return ((global::haxe.lang.Function) (new global::haxe.lang.Closure(this, "readUnsignedShort", 1997218321)) );
					}
					
					
					case 243225909:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return ((global::haxe.lang.Function) (new global::haxe.lang.Closure(this, "readBytes", 243225909)) );
					}
					
					
					case 2010580328:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return ((global::haxe.lang.Function) (new global::haxe.lang.Closure(this, "readUntil", 2010580328)) );
					}
					
					
					case 1115112725:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return ((global::haxe.lang.Function) (new global::haxe.lang.Closure(this, "readLinesUntil", 1115112725)) );
					}
					
					
					case 1873474154:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return ((global::haxe.lang.Function) (new global::haxe.lang.Closure(this, "readLine", 1873474154)) );
					}
					
					
					case 1224228525:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return ((global::haxe.lang.Function) (new global::haxe.lang.Closure(this, "readAllAvailableBytes", 1224228525)) );
					}
					
					
					case 1381630732:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return ((global::haxe.lang.Function) (new global::haxe.lang.Closure(this, "writeBytes", 1381630732)) );
					}
					
					
					case 602959184:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return ((global::haxe.lang.Function) (new global::haxe.lang.Closure(this, "writeInt", 602959184)) );
					}
					
					
					case 283737885:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return ((global::haxe.lang.Function) (new global::haxe.lang.Closure(this, "writeShort", 283737885)) );
					}
					
					
					case 1238832007:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return ((global::haxe.lang.Function) (new global::haxe.lang.Closure(this, "writeByte", 1238832007)) );
					}
					
					
					case 1152047462:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return this.chunks;
					}
					
					
					case 2062888707:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return this.currentData;
					}
					
					
					case 1134303404:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return this.currentOffset;
					}
					
					
					case 520590566:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return this.length;
					}
					
					
					case 2005096905:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return this.available;
					}
					
					
					default:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return base.__hx_getField(field, hash, throwErrors, isCheck, handleProperties);
					}
					
				}
				
			}
			#line default
		}
		
		
		public override double __hx_getField_f(string field, int hash, bool throwErrors, bool handleProperties) {
			unchecked {
				#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				switch (hash) {
					case 1134303404:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return ((double) (this.currentOffset) );
					}
					
					
					case 520590566:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return ((double) (this.length) );
					}
					
					
					case 2005096905:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return ((double) (this.available) );
					}
					
					
					default:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return base.__hx_getField_f(field, hash, throwErrors, handleProperties);
					}
					
				}
				
			}
			#line default
		}
		
		
		public override object __hx_invokeField(string field, int hash, object[] dynargs) {
			unchecked {
				#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				switch (hash) {
					case 1538685534:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return this.endsWith(global::haxe.lang.Runtime.toString(((object) (dynargs[0]) )));
					}
					
					
					case 1154061571:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return this.peekUntil(((int) (global::haxe.lang.Runtime.toInt(((object) (dynargs[0]) ))) ));
					}
					
					
					case 401708611:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return this.peekByte(((int) (global::haxe.lang.Runtime.toInt(((object) (dynargs[0]) ))) ));
					}
					
					
					case 1763375486:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return this.readByte();
					}
					
					
					case 89107268:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return this.readUnsignedInt();
					}
					
					
					case 1997218321:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return this.readUnsignedShort();
					}
					
					
					case 243225909:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return this.readBytes(((int) (global::haxe.lang.Runtime.toInt(((object) (dynargs[0]) ))) ));
					}
					
					
					case 2010580328:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return this.readUntil(global::haxe.lang.Runtime.toString(((object) (dynargs[0]) )));
					}
					
					
					case 1115112725:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return this.readLinesUntil(global::haxe.lang.Runtime.toString(((object) (dynargs[0]) )));
					}
					
					
					case 1873474154:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return this.readLine();
					}
					
					
					case 1224228525:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return this.readAllAvailableBytes();
					}
					
					
					case 1381630732:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						this.writeBytes(((global::haxe.io.Bytes) (((object) (dynargs[0]) )) ));
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						break;
					}
					
					
					case 602959184:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						this.writeInt(((int) (global::haxe.lang.Runtime.toInt(((object) (dynargs[0]) ))) ));
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						break;
					}
					
					
					case 283737885:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						this.writeShort(((int) (global::haxe.lang.Runtime.toInt(((object) (dynargs[0]) ))) ));
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						break;
					}
					
					
					case 1238832007:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						this.writeByte(((int) (global::haxe.lang.Runtime.toInt(((object) (dynargs[0]) ))) ));
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						break;
					}
					
					
					default:
					{
						#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
						return base.__hx_invokeField(field, hash, dynargs);
					}
					
				}
				
				#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
				return null;
			}
			#line default
		}
		
		
		public override void __hx_getFields(global::haxe.root.Array baseArr) {
			#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			baseArr.push("chunks");
			#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			baseArr.push("currentData");
			#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			baseArr.push("currentOffset");
			#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			baseArr.push("length");
			#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			baseArr.push("available");
			#line 5 "C:\\projects\\BCGAME\\sangong\\src\\hx\\ws\\Buffer.hx"
			base.__hx_getFields(baseArr);
		}
		#line default
		
	}
}


